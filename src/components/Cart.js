export default function Cart({
  ...
  const isNotEmpty = selectedDeserts.length > 0;
  return (
    <div className="cart">
      <h3>Your Cart({selectedDeserts.length})</h3>
      {isNotEmpty && (
        <>
          <ul>
            {selectedDeserts.map((desert) => (
              <li key={desert.name}>
                ...
                <span>${desert.count * desert.price}</span>
                <div
                  onClick={() => onDeleteDesert(desert.name)}
                  className="close"
                >
                  <svg
          ...
          <button onClick={() => setIsConfirmed(true)}>Confirm order</button>
        </>
      )}
